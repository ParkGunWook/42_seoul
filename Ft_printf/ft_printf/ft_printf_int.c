...
/*   Updated: 2021/05/10 21:56:44 by gpark            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "libprint.h"

static void		fill_pos_int_buffer(t_format *format_spec, char *print_buffer,
		const char *int_str, size_t buffer_size)
{
	char	to_fill;
	...
	if (format_spec->flags[ZERO_FLAG])
	{
		...
		print_buffer[buffer_size - 2 - ft_strlen(int_str)] = '+';
	print_buffer[buffer_size - 1] = 0;
}

static void		fill_neg_int_buffer(t_format *format_spec, char *print_buffer,
		const char *int_str, size_t buffer_size)
{
	char	to_fill;
	...
	{
		print_buffer[buffer_size - 1 - ft_strlen(int_str)] = to_fill;
		print_buffer[0] = '-';
	}
	print_buffer[buffer_size - 1] = 0;
}
...
int				ft_printf_int(t_format *format_spec, va_list ap)
{
	...
	write(1, "\n", 1);*/
	if (value < 0)
		fill_neg_int_buffer(format_spec, print_buffer, int_str, buffer_size);
	else
		fill_pos_int_buffer(format_spec, print_buffer, int_str, buffer_size);
